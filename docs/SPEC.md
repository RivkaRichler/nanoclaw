...
1. **Ensures Docker daemon is running** - Verifies Docker is available; kills orphaned NanoClaw containers from previous runs
...
| "Claude Code process exited with code 1" | Docker not running | Check logs; ensure Docker daemon is running (`docker info`) |
