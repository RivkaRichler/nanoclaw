...
NanoClaw deliberately limits which env vars reach the container. Only authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`) are extracted from `.env` and mounted for sourcing inside the container. Other env vars are not exposed.
...
docker info &>/dev/null && echo "OK" || echo "NOT RUNNING - start Docker (open -a Docker on macOS, systemctl start docker on Linux)"
