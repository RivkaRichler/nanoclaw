...
- Record PLATFORM and DOCKER values for step 3
...
- DOCKER=running → proceed to 3b
- DOCKER=installed_not_running → start Docker:
  - macOS: `open -a Docker`. Wait 15s, re-check with `docker info`. Poll a few more times if needed.
...
**Container agent fails ("Claude Code process exited with code 1"):** Ensure Docker is running (`docker info`). Start it: `open -a Docker` (macOS) or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.
